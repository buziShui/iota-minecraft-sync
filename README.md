...
- 同步码可绑定单个实例，也可创建适用于全部已发布实例的统一同步码；原码只显示一次，服务端仅保存 PBKDF2 加盐哈希。
...
将 MSLX Web/API 正在监听的 HTTP 端口提供给客户端，而不是 Minecraft 游戏端口。支持 PassNat、FRP、路由器端口映射、反向代理、内网地址或直接公网域名。玩家填写的同步源应是可访问的根地址，例如：
...
1. 选择“全部实例”创建统一同步码，或选择目标实例创建单实例同步码；建议使用玩家昵称作备注。
2. 立即复制并妥善保存页面显示的原始同步码；它只显示一次。
3. 将“同步服务根地址 + 同步码”私下发给对应玩家。

同步码长期有效。单实例码绑定创建时选择的服务端实例；统一同步码允许玩家在 PCL IO 中选择任意已发布实例。需要阻止某位玩家继续下载时，请先撤销对应同步码；撤销后可以永久删除该条管理记录。不要在截图、群公告或公开 Issue 中泄露同步码。
...
2. 点击添加同步源。
3. 输入管理员提供的同步服务根地址，例如 `http://example.com:12345`。
4. 输入管理员提供的长期同步码。
...
检查 MSLX 是否运行、所使用的端口映射或反向代理是否指向 MSLX Web/API 端口、防火墙是否放行，以及地址是否只填写根地址。还应确认同步码没有复制空格、没有被撤销；使用统一同步码时还需选择一个已发布实例。
...
可以。创建同步码时选择“全部实例”，玩家添加同步源后即可选择任意已发布实例。建议仍按玩家分别创建统一码，泄露或停用时才能只撤销一个人的权限。
...
使用统一同步码时，`GET source` 不带实例参数会返回可选实例列表；选定后，客户端在 `source`、`manifest` 和 `files/{path}` 请求中附加 `?instanceId=实例编号`。也可以通过 `X-Iota-Instance-Id` 请求头传递该编号。单实例码无需此参数，并继续兼容旧版客户端。
...
服务端插件的 Mod 端侧识别规则参考了 [xyblue135/ModSideChecker](https://github.com/xyblue135/ModSideChecker) 的元数据扫描思路，并在此基础上增加了 TOML 注释过滤、依赖块解析、冲突检测和管理员覆盖保护。第三方许可信息见 `THIRD_PARTY_NOTICES.md`。
