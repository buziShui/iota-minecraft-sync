...
#Region "合并安装"
    ...
    Public Class McInstallRequest
        ...
        ''' <summary>欲下载的 Quilt Loader 版本。</summary>
    ...
    Public Function McInstallLoader(Request As McInstallRequest) As List(Of LoaderBase)
        ...
        If Request.QuiltVersion IsNot Nothing Then FabricFolder = TempMcFolder & "versions\quilt-loader-" & Request.QuiltVersion & "-" & Request.MinecraftName
        ...
        Dim Modable As Boolean = Request.FabricVersion IsNot Nothing OrElse Request.QuiltVersion IsNot Nothing OrElse Request.ForgeEntry IsNot Nothing OrElse Request.NeoForgeEntry IsNot Nothing OrElse Request.LiteLoaderEntry IsNot Nothing
        ...
        If Request.QuiltVersion IsNot Nothing Then
            LoaderList.Add(New LoaderTask(Of Integer, Integer)("下载 Quilt " & Request.QuiltVersion,
                Sub(Task)
                    Directory.CreateDirectory(FabricFolder)
                    Dim profile = NetRequestByClientRetry($"https://meta.quiltmc.org/v3/versions/loader/{Request.MinecraftName}/{Request.QuiltVersion}/profile/json", RequireJson:=True)
                    FileUtils.Write(FabricFolder & "\" & PathUtils.GetLastPart(FabricFolder) & ".json", profile)
                End Sub) With {.Show = False, .ProgressWeight = 2, .Block = True})
        End If
        '合并安装
        ...
        '补全文件
        If Request.OptiFineEntry IsNot Nothing OrElse (Request.ForgeVersion IsNot Nothing AndAlso Request.ForgeVersion.BeforeFirst(".") >= 20) OrElse Request.NeoForgeVersion IsNot Nothing OrElse Request.FabricVersion IsNot Nothing OrElse Request.QuiltVersion IsNot Nothing OrElse Request.LiteLoaderEntry IsNot Nothing Then
            Dim LoadersLib As New List(Of LoaderBase)
