...
当前源码版本为 1.2.3，适配 MSLX 1.4.7，目标运行环境为 Linux x86_64。插件 ID：`mslx-plugin-iota-sync`。
...
1.2.3 修复 Native ESM 前端版本号滞后导致 MSLX 复用旧缓存的问题；保存配置或分类时固定保留当前扫描结果与发布记录，避免保存成功后列表消失、发布按钮无法使用。
